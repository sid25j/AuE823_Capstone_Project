...
class LineFollower(object):
    ...
    def __init__(self):
        ...
        #self.tag_sub = rospy.Subscriber('/detect_apriltag', Int8, self.tag_callback)
        self.switch = 0
        self.delta = 0
        self.flag = 0 
        #self.end_of_lane = 1
    ...
    def stop_detection(self,msg):
        ...
        if self.flag != 1:
            ...
            stop_detected = 0
        #self.delta = stop_detected_old - stop_detected
    ...
    def scan_data(self,distance):
        ...
        #print(self.switch)
        if self.switch == 0:
            if self.tag_detected == 0:
                #self.end_of_lane = 1
                distance.ranges = [x if x<3.5 else 3.5 for x in distance.ranges]
    ...
    def camera_callback(self, data):
        ...
        if m['m00'] == 0:
            self.switch = 0
            #print("Obstacle avoid/ follow april tag")
        else:
            self.switch = 1
            #print("Line follow")
        ...
        if self.switch == 1:
            ...
            if (stop_detected==0):
                ...
                vel_msg.angular.y = 0
                self.velocity_publisher.publish(vel_msg)
            elif self.flag == 0: #stop_detected==1:
                print("Stop Detected")
                vel_msg.linear.x = 0
                vel_msg.linear.y = 0
                vel_msg.linear.z = 0
